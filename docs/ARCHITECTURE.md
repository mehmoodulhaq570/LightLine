...
The Windows prototype is now split by responsibility. `src/main.rs` is only the entry point; `src/windows_app/window.rs` owns window creation and Win32 message routing. `app.rs` owns editor state and common commands, `workspace.rs` owns the lazy file tree, `panels.rs` owns on-demand search/run/review work, and `input.rs` handles keyboard and mouse actions. Rendering lives under `render/` with editor painting, panel painting, and shared GDI primitives in separate files. `icons.rs` owns file icons and the LightLine app icon. The text model (`src/document.rs`), syntax (`src/syntax.rs`), and workspace services (`src/workflow.rs`) remain outside the Windows UI. This is a responsibility split rather than a line-count rule; the UI still shares one `App` state and can be refined as new features make ownership boundaries clearer.
