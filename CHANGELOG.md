...
- **Extensions Marketplace Redesign**: Rebuilt the native Extensions sidebar with a larger search field, Marketplace/Installed tabs, Featured/Popular/Recently Updated filters, recommended-count heading, richer cards for Prettier, Material Icon Theme and Dracula, and a compact Active Capabilities summary. Search and filters use the live extension model, install/installed buttons retain their real behavior, registry versions refresh in place, and capability rows report the formatter and icon theme that are actually active.
