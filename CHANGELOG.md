...
- Pyright now sets itself up: when `pyright-langserver` is not on PATH, LightLine silently installs Pyright into `%APPDATA%\LightLine\pyright` on first use (a one-time background download via npm) and launches it with Node from there, showing setup progress in the status bar — no manual `npm install` needed. A global install still takes precedence, and if Node.js itself is missing the status bar explains that instead of a raw process error. Covered by resolver and message unit tests.
