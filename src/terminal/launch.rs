...
pub enum LaunchRequest {
    Shell { cwd: PathBuf, no_profile: bool },
    Python { interpreter: PathBuf, script: PathBuf, cwd: PathBuf, no_profile: bool },
}

impl LaunchRequest {
    pub fn shell(cwd: PathBuf) -> Self {
        Self::Shell { cwd, no_profile: false }
    }

    pub fn python(interpreter: PathBuf, script: PathBuf, cwd: PathBuf) -> Self {
        Self::Python { interpreter, script, cwd, no_profile: false }
    }
...
pub fn resolve_powershell() -> Result<PathBuf, String> {
    ...
    if let Some(root) = root {
        let path = PathBuf::from(root).join("System32").join("WindowsPowerShell").join("v1.0").join("powershell.exe");
...
fn prepare_with_shell(request: &LaunchRequest, executable: PathBuf) -> Result<LaunchSpec, String> {
    let (cwd, no_profile) = match request {
        LaunchRequest::Shell { cwd, no_profile } | LaunchRequest::Python { cwd, no_profile, .. } => (cwd, *no_profile),
    };
    ...
    if no_profile { spec.arguments.push(OsString::from("-NoProfile")); }
    if let LaunchRequest::Python { interpreter, script, .. } = request {
        ...
        spec.arguments.extend([OsString::from("-NoExit"), OsString::from("-EncodedCommand"), OsString::from(encode_powershell_command(&command))]);
...
fn absolute_existing(path: &Path, directory: bool) -> Result<PathBuf, String> {
    if (directory && !path.is_dir()) || (!directory && !path.is_file()) {
        return Err(format!("{} does not exist or has the wrong type", path.display()));
    }
    reject_nul(path.as_os_str())?;
    std::path::absolute(path).map_err(|error| format!("Could not resolve {}: {error}", path.display()))
}
...
pub(super) fn reject_nul(value: &OsStr) -> Result<(), String> {
    if os_wide(value).contains(&0) { Err("Paths and arguments cannot contain NUL".into()) } else { Ok(()) }
}
...
fn base64(bytes: &[u8]) -> String {
    ...
    for chunk in bytes.chunks(3) {
        ...
        result.push(if chunk.len() > 1 { ALPHABET[((word >> 6) & 63) as usize] as char } else { '=' });
        result.push(if chunk.len() > 2 { ALPHABET[(word & 63) as usize] as char } else { '=' });
    }
    result
}

pub fn encode_powershell_command(command: &str) -> String {
    base64(&command.encode_utf16().flat_map(u16::to_le_bytes).collect::<Vec<_>>())
}
...
pub fn powershell_path_expression(path: &Path) -> Result<String, String> {
    reject_nul(path.as_os_str())?;
    let bytes: Vec<_> = os_wide(path.as_os_str()).into_iter().flat_map(u16::to_le_bytes).collect();
    Ok(format!("([System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String('{}')))", base64(&bytes)))
}
...
pub fn python_command(interpreter: &Path, script: &Path, cwd: &Path) -> Result<String, String> {
    ...
    Ok(format!("Set-Location -LiteralPath {cwd} -ErrorAction Stop; & {interpreter} '-u' {script}"))
}

fn python_environment(interpreter: &Path, inherited_path: Option<&OsStr>) -> Result<Vec<(OsString, Option<OsString>)>, String> {
    let directory = interpreter.parent().ok_or("Python interpreter needs a parent directory")?;
    let is_scripts = directory.file_name().is_some_and(|name| name.to_string_lossy().eq_ignore_ascii_case("Scripts"));
    let scripts = if is_scripts { directory.to_path_buf() } else { directory.join("Scripts") };
    let mut paths = vec![directory.to_path_buf()];
    if scripts != directory { paths.push(scripts); }
    if let Some(path) = inherited_path { paths.extend(std::env::split_paths(path)); }
    let path = std::env::join_paths(paths).map_err(|error| format!("Invalid child PATH: {error}"))?;
    let venv = if is_scripts {
        directory.parent().filter(|root| root.join("pyvenv.cfg").is_file()).map(|root| root.as_os_str().to_os_string())
    } else { None };
    Ok(vec![
...
pub fn quote_windows_argument(argument: &OsStr) -> Result<Vec<u16>, String> {
    ...
    for unit in os_wide(argument) {
        if unit == u16::from(b'\\') { slashes += 1; continue; }
        if unit == u16::from(b'"') {
...
mod tests {
    ...
    fn decode(encoded: &str) -> String {
        ...
        for chunk in encoded.as_bytes().chunks(4) {
            let mut value = 0u32;
            for b in chunk { value = (value << 6) | alphabet.iter().position(|v| v == b).unwrap_or(0) as u32; }
            bytes.push((value >> 16) as u8);
            if chunk[2] != b'=' { bytes.push((value >> 8) as u8); }
            if chunk[3] != b'=' { bytes.push(value as u8); }
        }
        String::from_utf16(&bytes.chunks_exact(2).map(|pair| u16::from_le_bytes([pair[0], pair[1]])).collect::<Vec<_>>()).unwrap()
    }
    ...
    fn terminal_python_paths_are_data_not_powershell_source() {
        ...
        let command = python_command(Path::new("C:\\Python\\python.exe"), malicious, Path::new("C:\\work [1]")).unwrap();
    ...
    fn terminal_windows_argument_quoting() {
        let quote = |s: &str| String::from_utf16(&quote_windows_argument(OsStr::new(s)).unwrap()).unwrap();
        assert_eq!(quote(""), "\"\"");
    ...
    fn terminal_no_profile_is_explicit_recovery() {
        ...
        assert!(matches!(request, LaunchRequest::Shell { no_profile: false, .. }));
        assert!(matches!(request.without_profile(), LaunchRequest::Shell { no_profile: true, .. }));
    }

    #[test]
    fn terminal_venv_requires_config_next_to_scripts() {
        let nonce = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
        let root = std::env::temp_dir().join(format!("lightline-venv-{}-{nonce}", std::process::id()));
        ...
        assert_eq!(std::env::split_paths(environment[0].1.as_ref().unwrap()).collect::<Vec<_>>(), [root.join("Scripts")]);
    }
    ...
    fn terminal_unverified_venv_is_not_inherited() {
        ...
        assert_eq!(std::env::split_paths(path).collect::<Vec<_>>(), [root.clone(), root.join("Scripts")]);
    }
}
