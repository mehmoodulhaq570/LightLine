...
use windows_sys::Win32::Foundation::{CloseHandle, ERROR_BROKEN_PIPE, ERROR_OPERATION_ABORTED, HANDLE, INVALID_HANDLE_VALUE, WAIT_OBJECT_0, WAIT_TIMEOUT};
use windows_sys::Win32::Storage::FileSystem::{ReadFile, WriteFile};
use windows_sys::Win32::System::Console::{COORD, HPCON};
use windows_sys::Win32::System::IO::CancelSynchronousIo;
use windows_sys::Win32::System::JobObjects::{AssignProcessToJobObject, CreateJobObjectW, JOBOBJECT_EXTENDED_LIMIT_INFORMATION, JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE, JobObjectExtendedLimitInformation, SetInformationJobObject, TerminateJobObject};
use windows_sys::Win32::System::LibraryLoader::{GetModuleHandleW, GetProcAddress};
use windows_sys::Win32::System::Pipes::CreatePipe;
use windows_sys::Win32::System::Threading::{CREATE_SUSPENDED, CREATE_UNICODE_ENVIRONMENT, CreateProcessW, DeleteProcThreadAttributeList, EXTENDED_STARTUPINFO_PRESENT, GetCurrentThreadId, GetExitCodeProcess, INFINITE, InitializeProcThreadAttributeList, LPPROC_THREAD_ATTRIBUTE_LIST, OpenThread, PROCESS_INFORMATION, PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE, ResumeThread, STARTUPINFOEXW, THREAD_TERMINATE, TerminateProcess, UpdateProcThreadAttribute, WaitForSingleObject};
...
impl Handle {
    fn new(raw: HANDLE, operation: &str) -> Result<Self, String> {
        if raw.is_null() || raw == INVALID_HANDLE_VALUE { Err(last_error(operation)) } else { Ok(Self(raw)) }
    }
}
...
fn pipe() -> Result<(Handle, Handle), String> {
    ...
    if unsafe { CreatePipe(&mut read, &mut write, null(), 0) } == 0 { return Err(last_error("CreatePipe")); }
...
impl PseudoConsole {
    fn new(size: TerminalSize, input: &Handle, output: &Handle) -> Result<Self, String> {
        ...
        unsafe {
            ...
            if module.is_null() { return Err(last_error("GetModuleHandleW")); }
            let create = GetProcAddress(module, c"CreatePseudoConsole".as_ptr().cast())
                .ok_or("ConPTY requires Windows 10 version 1809 or newer")?;
            let resize = GetProcAddress(module, c"ResizePseudoConsole".as_ptr().cast()).ok_or("ResizePseudoConsole is unavailable")?;
            let close = GetProcAddress(module, c"ClosePseudoConsole".as_ptr().cast()).ok_or("ClosePseudoConsole is unavailable")?;
            ...
            if result < 0 { return Err(format!("CreatePseudoConsole failed: HRESULT {result:#010x}")); }
    ...
    fn resize(&self, size: TerminalSize) -> Result<(), String> {
        ...
        if result < 0 { Err(format!("ResizePseudoConsole failed: HRESULT {result:#010x}")) } else { Ok(()) }
    }
}

impl Drop for PseudoConsole {
    fn drop(&mut self) { unsafe { (self.close)(self.raw); } }
}

fn coord(size: TerminalSize) -> COORD { COORD { X: size.columns as i16, Y: size.rows as i16 } }
...
impl Attributes {
    fn new(console: HPCON) -> Result<Self, String> {
        ...
        if bytes == 0 { return Err(last_error("Size process attributes")); }
        ...
        if unsafe { UpdateProcThreadAttribute(attributes.pointer(), 0, PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE as usize, console as *const c_void, size_of::<HPCON>(), null_mut(), null()) } == 0 {
            return Err(last_error("UpdateProcThreadAttribute(PSEUDOCONSOLE)"));
        }
        Ok(attributes)
    }

    fn pointer(&mut self) -> LPPROC_THREAD_ATTRIBUTE_LIST { self.storage.as_mut_ptr().cast() }
}

impl Drop for Attributes {
    fn drop(&mut self) { unsafe { DeleteProcThreadAttributeList(self.pointer()); } }
}
...
fn environment(spec: &LaunchSpec) -> Result<Vec<u16>, String> {
    ...
    for (key, value) in &spec.environment {
        reject_nul(key)?;
        if key.is_empty() || key.to_string_lossy().contains('=') { return Err("Invalid child environment key".into()); }
        entries.retain(|(existing, _)| !existing.to_string_lossy().eq_ignore_ascii_case(&key.to_string_lossy()));
        if let Some(value) = value { entries.push((key.clone(), value.clone())); }
    }
...
fn create_job() -> Result<Handle, String> {
    let job = Handle::new(unsafe { CreateJobObjectW(null(), null()) }, "CreateJobObjectW")?;
    let mut limits = JOBOBJECT_EXTENDED_LIMIT_INFORMATION::default();
    limits.BasicLimitInformation.LimitFlags = JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE;
    if unsafe { SetInformationJobObject(job.0, JobObjectExtendedLimitInformation, (&limits as *const JOBOBJECT_EXTENDED_LIMIT_INFORMATION).cast(), size_of_val(&limits) as u32) } == 0 {
...
fn spawn(spec: &LaunchSpec, console: &PseudoConsole, job: &Handle) -> Result<Process, String> {
    ...
    command.push(0);
    if command.len() > 32_767 { return Err("Terminal command line exceeds the Windows limit".into()); }
    ...
    if unsafe { CreateProcessW(executable.as_ptr(), command.as_mut_ptr(), null(), null(), 0, flags, environment.as_ptr().cast(), directory.as_ptr(), &startup.StartupInfo, &mut information) } == 0 {
        return Err(last_error("CreateProcessW"));
    }
    let process = Process { process: Handle(information.hProcess), thread: Handle(information.hThread) };
    if unsafe { AssignProcessToJobObject(job.0, process.process.0) } == 0 {
        return Err(last_error("AssignProcessToJobObject (process was not resumed)"));
    }
    if unsafe { ResumeThread(process.thread.0) } == u32::MAX { return Err(last_error("ResumeThread")); }
    Ok(process)
}
...
impl IoWorker {
    fn spawn(name: &str, shared: Arc<Shared>, work: impl FnOnce() + Send + 'static) -> Result<Self, String> {
        ...
        let thread = thread::Builder::new().name(name.into()).spawn(move || {
            ...
            let handle = Handle::new(unsafe { OpenThread(THREAD_TERMINATE, 0, GetCurrentThreadId()) }, "OpenThread for I/O cancellation");
            let ready = handle.is_ok();
            if ready_tx.send(handle).is_err() || !ready { return; }
        ...
        match ready_rx.recv() {
            Ok(Ok(native_thread)) => Ok(Self { thread: Some(thread), native_thread }),
            Ok(Err(error)) => { let _ = thread.join(); Err(error) }
            Err(error) => { let _ = thread.join(); Err(format!("Terminal I/O startup: {error}")) }
        }
    }

    fn finished(&self) -> bool { self.thread.as_ref().is_none_or(JoinHandle::is_finished) }

    fn cancel(&self) { unsafe { CancelSynchronousIo(self.native_thread.0); } }

    fn join(&mut self) { if let Some(thread) = self.thread.take() { let _ = thread.join(); } }
}
...
impl Runtime {
    fn new(shared: Arc<Shared>) -> Self {
        Self { shared, console: None, job: None, process: None, reader: None, writer: None }
    }
}
...
fn read_output(pipe: Handle, shared: Arc<Shared>, output: SyncSender<Vec<u8>>) {
    ...
    loop {
        if shared.reader_abort.load(Ordering::Acquire) { break; }
        let mut count = 0;
        let ok = unsafe { ReadFile(pipe.0, buffer.as_mut_ptr(), buffer.len() as u32, &mut count, null_mut()) };
        if ok == 0 {
        ...
        loop {
            match output.try_send(bytes) {
                ...
                Err(TrySendError::Full(pending)) => {
                    if shared.reader_abort.load(Ordering::Acquire) { return; }
                    bytes = pending;
...
fn write_bytes(pipe: &Handle, shared: &Shared, bytes: &[u8]) -> bool {
    for chunk in bytes.chunks(4096) {
        ...
        while !remaining.is_empty() {
            if shared.io_stop.load(Ordering::Acquire) { return false; }
            let mut count = 0;
            let ok = unsafe { WriteFile(pipe.0, remaining.as_ptr(), remaining.len() as u32, &mut count, null_mut()) };
            if ok == 0 || count == 0 {
                if !shared.io_stop.load(Ordering::Acquire) { shared.fail(last_error("Terminal write")); }
                return false;
            }
...
fn write_input(pipe: Handle, shared: Arc<Shared>, input: Receiver<Vec<u8>>, replies: Receiver<Vec<u8>>) {
...
pub(super) fn run(spec: LaunchSpec, size: TerminalSize, shared: Arc<Shared>, input: Receiver<Vec<u8>>, output: SyncSender<Vec<u8>>, replies: Receiver<Vec<u8>>) -> SessionStatus {
    ...
    let failure = shared.failure.lock().unwrap_or_else(|e| e.into_inner()).clone();
    if let Some(error) = failure { return SessionStatus::Failed(error); }
    match result {
...
fn run_inner(spec: LaunchSpec, size: TerminalSize, shared: Arc<Shared>, input: Receiver<Vec<u8>>, output: SyncSender<Vec<u8>>, replies: Receiver<Vec<u8>>) -> Result<Option<u32>, String> {
    ...
    runtime.reader = Some(IoWorker::spawn("terminal-reader", shared.clone(), move || read_output(host_output, reader_shared, output))?);
    let writer_shared = shared.clone();
    runtime.writer = Some(IoWorker::spawn("terminal-writer", shared.clone(), move || write_input(host_input, writer_shared, input, replies))?);
    runtime.job = Some(create_job()?);
    runtime.process = Some(spawn(&spec, runtime.console.as_ref().unwrap(), runtime.job.as_ref().unwrap())?);
    shared.set_status(SessionStatus::Running);
    loop {
        if shared.stop.load(Ordering::Acquire) { return Ok(None); }
        let process = &runtime.process.as_ref().unwrap().process;
        match unsafe { WaitForSingleObject(process.0, 0) } {
            WAIT_OBJECT_0 => {
                let mut code = 0;
                if unsafe { GetExitCodeProcess(process.0, &mut code) } == 0 { return Err(last_error("GetExitCodeProcess")); }
                return Ok(Some(code));
            }
            WAIT_TIMEOUT => {}
            _ => return Err(last_error("WaitForSingleObject")),
        }
        if shared.output_eof.load(Ordering::Acquire) { return Err("Terminal output closed while the shell was running".into()); }
        let desired = shared.desired_size.load(Ordering::Acquire);
        if desired != shared.actual_size.load(Ordering::Acquire) {
            runtime.console.as_ref().unwrap().resize(TerminalSize::unpack(desired))?;
...
mod tests {
    ...
    fn await_snapshot(service: &mut TerminalService, id: crate::terminal::SessionId, predicate: impl Fn(&crate::terminal::Snapshot) -> bool) -> Arc<crate::terminal::Snapshot> {
        let deadline = Instant::now() + Duration::from_secs(20);
        loop {
            if let Some(snapshot) = service.snapshot(id) && predicate(&snapshot) { return snapshot; }
            for event in service.poll(2) {
                if event.session_id == id && predicate(&event.snapshot) { return event.snapshot; }
            }
            assert!(Instant::now() < deadline, "Timed out waiting for terminal session {id:?}");
    ...
    fn terminal_live_shell_resize_input_exit_and_stale_id() {
        ...
        let id = service.start(SessionKind::Shell, request, TerminalSize::new(12, 80).unwrap()).unwrap();
        await_snapshot(&mut service, id, |s| s.status == SessionStatus::Running);
        service.resize(id, TerminalSize::new(15, 90).unwrap()).unwrap();
        service.input(id, b"[Console]::WriteLine('CONPTY_' + 'READY')\r").unwrap();
        let snap = await_snapshot(&mut service, id, |s| s.text().contains("CONPTY_READY") && s.size.columns == 90);
    ...
    fn terminal_live_independent_shells_and_flood_stop() {
        let mut service = TerminalService::default();
        let request = LaunchRequest::shell(std::env::current_dir().unwrap()).without_profile();
        let shell = service.start(SessionKind::Shell, request.clone(), TerminalSize::default()).unwrap();
        let run = service.start(SessionKind::ManagedRun, request, TerminalSize::default()).unwrap();
        ...
        if !service.snapshot(run).is_some_and(|s| s.status == SessionStatus::Running) {
            await_snapshot(&mut service, run, |s| s.status == SessionStatus::Running);
        }
        service.input(run, b"while ($true) { [Console]::WriteLine(('x' * 200)) }\r").unwrap();
        ...
        assert_eq!(service.snapshot(shell).unwrap().status, SessionStatus::Running);
    ...
    fn terminal_live_stop_kills_descendant() {
        ...
        let mut service = TerminalService::default();
        let request = LaunchRequest::shell(std::env::current_dir().unwrap()).without_profile();
        let id = service.start(SessionKind::Shell, request, TerminalSize::default()).unwrap();
        await_snapshot(&mut service, id, |s| s.status == SessionStatus::Running);
        ...
        let child_pid = |snapshot: &crate::terminal::Snapshot| {
            snapshot.text().lines().find_map(|line| line.trim().strip_prefix("CHILD_").and_then(|id| id.parse::<u32>().ok()))
        };
        let snapshot = await_snapshot(&mut service, id, |s| child_pid(s).is_some());
        let child = Handle::new(unsafe { OpenProcess(PROCESS_SYNCHRONIZE, 0, child_pid(&snapshot).unwrap()) }, "Open child for lifecycle test").unwrap();
        service.stop(id).unwrap();
        await_snapshot(&mut service, id, |s| s.status.is_final());
        assert_eq!(unsafe { WaitForSingleObject(child.0, 2_000) }, WAIT_OBJECT_0);
        service.remove(id).unwrap();
    }
    ...
    fn terminal_live_python_input_safe_paths_and_retained_shell() {
        let interpreter = PathBuf::from(std::env::var_os("LIGHTLINE_TEST_PYTHON").expect("Set LIGHTLINE_TEST_PYTHON to the selected python.exe"));
        ...
        std::fs::write(&script, "print('PY_READY', flush=True)\nvalue = input()\nprint('GOT_' + value, flush=True)\n").unwrap();
        let mut service = TerminalService::default();
        let request = LaunchRequest::python(interpreter, script, root).without_profile();
        let id = service.start(SessionKind::ManagedRun, request, TerminalSize::default()).unwrap();
        ...
        service.input(id, b"Write-Output ('SHELL_' + 'ALIVE')\r").unwrap();
        await_snapshot(&mut service, id, |s| s.text().contains("SHELL_ALIVE") && s.status == SessionStatus::Running);
        service.stop(id).unwrap();
        await_snapshot(&mut service, id, |s| s.status.is_final());
        service.remove(id).unwrap();
    }
    ...
    fn terminal_live_failed_spawn_reaps_pipe_workers() {
        ...
        let wake = Arc::new(crate::terminal::Wake { pending: std::sync::atomic::AtomicBool::new(false), callback: Arc::new(|| {}) });
        ...
        let spec = LaunchSpec { executable: cwd.join("lightline-intentionally-missing-shell.exe"), arguments: vec![], cwd, environment: vec![] };
    ...
    fn terminal_live_missing_cwd_reports_failure() {
        ...
        let id = service.start(SessionKind::Shell, LaunchRequest::shell(missing), TerminalSize::default()).unwrap();
        let snap = await_snapshot(&mut service, id, |s| s.status.is_final());
