...
- 🌿 **Source Control**: Stage, commit and discard from the sidebar (`Ctrl+Shift+G`). A focused commit composer, Push/Pull/Fetch controls, branch and sync status, polished clean-worktree state, timeline commit history, separate Staged and Changes lists, and side-by-side diff review keep the complete Git workflow inside LightLine.
