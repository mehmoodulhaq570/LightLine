...
LightLine supports UTF-8 files, editing, undo/redo, saving, multiple tabs, vertical split panes, Rust and Python syntax coloring, rust-analyzer diagnostics, hover, go-to-definition, completion and formatting, Pyright diagnostics, hover, go-to-definition, completion and formatting for Python, an integrated terminal, workspaces, project search, Quick Open, Rust test output, read-only Git review, and read-only image and hex-dump previews for non-text files. It paints only visible document lines.
...
Errors and warnings appear in the gutter and under the code; move the cursor onto a marked line to read its message in the status bar. Press F1 or pause the mouse over Rust or Python code for hover information, F12 to jump to the definition of the symbol under the cursor, and Shift+Alt+F to format the file. Press Ctrl+Space to open a completion list at the caret, use Up/Down to browse it, Enter or Tab to accept, and Esc to dismiss. Edits are sent incrementally, and saving sends a save notification so diagnostics can refresh. Split panes showing the same file share one document and one LSP update. Each server runs separately from the UI; if it is slow or exits, editing continues and the status bar shows the error.
